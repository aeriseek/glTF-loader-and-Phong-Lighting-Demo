...
std::vector<float> Model::getFloats(json accessor)
{
	...
	else throw std::invalid_argument("Type unknown");
...
std::vector<GLuint> Model::getIndices(json accessor)
{
	...
	unsigned int lengthOfData = count * bytesPerIndex;
...
std::vector<Vertex> Model::assembleVertices(std::vector<glm::vec3> positions, std::vector<glm::vec3> normals, std::vector<glm::vec2> texUVs)
{
	...
	for (int i = 0; i < positions.size(); i++)
		vertices.push_back({ positions[i], normals[i], glm::vec3(1.0f, 1.0f, 1.0f), texUVs[i] });
	return vertices;
}
